...
def check_login():
    ...
    if username == "user" and password == "password":
        ...
        nav_bar.add_cascade(label="System", menu=file_menu)
        nav_bar.add_cascade(label="Instance")
        nav_bar.add_cascade(label="Windows")
        nav_bar.add_cascade(label="Help")
        ...
        if __name__ == "__main__":
            ...
            messagebox.showinfo(title="Echec de la connexion", message="Identifiants incorrects")
...
root.iconbitmap("/Alternance-Copernilabs/Approche-du-problème/Stream/coper.ico")

frame = ctk.CTkFrame(master=root)
frame.pack(pady=20, padx=40, fill="both", expand=True)

username_label = ctk.CTkLabel(root, text="Nom d'utilisateur:")
username_label.pack()

username_entry = ctk.CTkEntry(root)
username_entry.pack()

password_label = ctk.CTkLabel(root, text="Mot de passe:")
password_label.pack()

password_entry = ctk.CTkEntry(root, show="*")
password_entry.pack()

login_button = ctk.CTkButton(root, text="Se Connecter", command=check_login)
login_button.pack()
